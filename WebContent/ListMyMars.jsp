 ...
 <form action="/uta_facility_maintenance_system/MarReportController" method="post">          
       <table border="1" class="myTable"> 
 		...
 		<c:forEach items="${MyRepairList}" var="item" varStatus="status">
			...
			<td class="myTableCell" style="padding-right: 35px; "><c:out value="${item.reportedBy}" /></td>
			<td class="myTableCell" style="padding-right: 35px; "><c:out value="${item.createdDate}" /></td>
			<td class="myTableCell" style="padding-right: 35px; "><c:out value="${item.marNumber}" /></td>
			
			<td> <a href="/uta_facility_maintenance_system/MarReportController?action=viewMyMar&id=${item.marNumber}">View</a></td>
			</tr>
		</c:forEach>
 </table>
